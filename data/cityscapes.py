...
def image_to_label(image):
    ...
    for i in range(0, class_count):
        feature = np.all(image == (selected_labels[i].color[2], selected_labels[i].color[1], selected_labels[i].color[0]), axis=-1)
        label[i] = feature[:, :] / 1.0
...
def classes_to_words(classes):
    ...
    for i in range(0, class_count):
        if (classes[i] == 1):
            words.append(selected_labels[i].name)
    
    return words

def words_to_classes(words):
    words = words.split('|')
    classes = np.zeros(shape=(class_count))
    for i in range(0, class_count):
        if selected_labels[i].name in words:
            classes[i] = 1
    return classes