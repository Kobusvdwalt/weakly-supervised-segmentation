if __name__ == '__main__':
    ...
    for class_name in class_list_classification:
        ...
        print('***********************')
        # Set up dataloader
        pascal_train = PascalVOCClassificationBinary(source='train', target=target_class)
        pascal_val = PascalVOCClassificationBinary(source='val', target=target_class)
        ...
        def train_model(model, criterion, optimizer, scheduler, num_epochs=11):
            best_acc = 0.0
            for epoch in range(num_epochs):
                ...
                for phase in ['train', 'val']:
                    ...
                    batch_count = 0
                    for inputs, labels, imageName in dataloaders[phase]:
                        batch_count += 1
                        ...
                        accuracy_m = np.sum(accuracy_np) / 2
                        precision_m = np.sum(precision_np) / 2
                        recall_m = np.sum(recall_np) / 2
                        f1_m = np.sum(f1_np) / 2
