...
class CAM(object):
    ...
    def forward(self, x, idx=None):
        ...
        weight_fc = list(self.model._modules.get('dense1').parameters())[0].to('cpu').data

        # object classification
        score = self.model(x)

        # threshold across classes
        cams = []
        for class_id in range(0, len(score[0])):
            cam = self.getCAM(self.values, weight_fc, class_id)
            cams.append({
                'class_id': class_id,
                'cam': cam
            })
...
class GradCAM(CAM):
    ...
    def forward(self, x, idx=None):
        ...
        cams = []
        for class_id in range(0, len(score[0])):
            cam = self.getGradCAM(self.values, score, class_id)
            cams.append({
                'class_id': class_id,
                'cam': cam
            })
...
class GradCAMpp(CAM):
    ...
    def forward(self, x, idx=None):
        ...
        cams = []
        for class_id in range(0, len(score[0])):
            cam = self.getGradCAMpp(self.values, score, class_id)
            cams.append({
                'class_id': class_id,
                'cam': cam
            })
...
class Gradients(CAM):
    ...
    def forward(self, x, idx=None):
        ...
        cams = []
        for class_id in range(0, len(score[0])):
            cam = self.getGradients(self.values, score, class_id)
            cams.append({
                'class_id': class_id,
                'cam': cam,
            })
    ...
    def getGradients(self, values, score, idx):
        ...
        gradients -= torch.min(gradients)
        gradients /= torch.max(gradients) + 1e-7
