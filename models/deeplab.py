...
class DeepLab(ModelBase):
    ...
    def event(self, event):
        ...
        if event['name'] == 'minibatch' and event['phase'] == 'train':
            image_cu = event['inputs']['image'].cuda(non_blocking=True)
            label_cu = event['labels']['pseudo'].cuda(non_blocking=True)
            ...
            if self.training:
                ...
                if event['batch'] % 100 == 0:
                    ...
                    for i in range(0, batch_size):
                        ...
                        log['miou'] += metrics.jaccard_score(np.argmax(content_label, 0).flatten(), np.argmax(content_predi, 0).flatten(), average='macro')
